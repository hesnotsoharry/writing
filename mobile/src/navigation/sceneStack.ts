...
/**
 * The editor stays top-of-stack (iOS interactive-pop is separately disabled
 * via gestureEnabled: false), but Android hardware/gesture back must pop to
 * the Hub, not finish the activity. Verified on the API 36 emulator
 * 2026-08-09: a bare single-route reset made the back gesture exit the app
 * from the editor. Seeding the logical parent chain under the Scene entry
 * gives back a sane path: Scene -> Hub -> Projects.
 */
