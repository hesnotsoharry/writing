...
describe("note promotion", () => {
  it("uses a stable note idempotency key and does not create a duplicate on replay", async () => {
    ...
    const deps = { findSceneByKey: vi.fn(async () => existing), createScene, saveSceneDoc: vi.fn(async () => undefined), publishScene: vi.fn(async () => undefined), markFiled: vi.fn(async () => undefined), syncAfterSave: vi.fn(async () => undefined) };
  ...
  it("marks the note filed only after the scene doc exists and is published", async () => {
    const order: string[] = [];
    const deps = { findSceneByKey: async () => null, createScene: async () => { order.push("meta"); return "scene"; }, saveSceneDoc: async () => { order.push("doc"); }, publishScene: async () => { order.push("publish"); }, markFiled: async () => { order.push("filed"); }, syncAfterSave: async () => { order.push("sync"); } };
