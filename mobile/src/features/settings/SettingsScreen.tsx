  ...
  return <Screen scroll contentStyle={styles.screen}><Topbar title="Settings" />
    <View style={styles.content}><SyncCard deviceName={deviceName} status={status}
      ...
      <Group label="Assistant"><Toggle label="AI features" description="Inherited access from desktop; this switch is local to this phone." value={settings.aiEnabled} onChange={(value) => update("aiEnabled", value)} />
        ...
        <Hairline /><ListRow title="Balance" meta={balance} trailing={<Text style={[TYPE.meta, { color: theme.colors.accent }]}>Top up</Text>} onPress={() => navigation.navigate("AiLimits", { projectId: route.params?.projectId ?? "", reason: "out-of-credit" })} /></Group>
      ...
      <Text style={[TYPE.metaSmall, styles.footer, { color: theme.colors.ink3 }]}>Theme, prose size, spell check and focus choices stay on this device. Compile/export, replace-across-scenes, label and entity-type definition, and BYOK API-key entry stay on desktop.</Text>
    </View></Screen>;
}
