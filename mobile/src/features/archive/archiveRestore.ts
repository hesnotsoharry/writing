...
function sceneFrom(raw: unknown, fallbackId: string, fallbackTitle: string, folderId: string | null): RestoredScene {
  const row = object(raw); const meta = object(row.meta);
  return {
    id: text(row.id, fallbackId), title: text(row.title, fallbackTitle), folderId,
    synopsis: nullableText(row.synopsis ?? meta.synopsis), status: text(row.status ?? meta.status, "blank"),
    sortOrder: number(row.sortOrder ?? meta.sort_order, 1000),
    wordCount: number(row.wordCount ?? meta.word_count, 0),
...
export function parseArchiveManifest(payload: ArchivePayload): ArchiveRestorePlan {
  ...
  if (payload.kind === "scene") {
    ...
    return { archiveId: payload.id, projectId: payload.projectId, kind: "scene", folder: null, scenes: [sceneFrom(manifest, id, payload.title, null)] };
  }
  const folderId = payload.originalId ?? crypto.randomUUID(); const folder = object(manifest.folder);
  const sortOrder = number(manifest.folderSortOrder ?? folder.sort_order, 1000);
  const entries = Array.isArray(manifest.scenes) ? manifest.scenes : [];
  return { archiveId: payload.id, projectId: payload.projectId, kind: "chapter", folder: { id: folderId, title: payload.title, sortOrder }, scenes: entries.map((entry) => sceneFrom(entry, crypto.randomUUID(), "Untitled", folderId)) };
}
