...
import { getBinderStore, getEpochOwner, getSceneDocStore, getSnapshotStore } from "../../db/stores";
import type { RootStackParamList } from "../../navigation/routes";
import { applyEncoded, encodeDoc, extractPlainText } from "../../shared/serialize";
...
async function updateRestoredWordCount(sceneId: string, stateBase64: string): Promise<void> {
  const text = plainText(stateBase64);
  const binder = await getBinderStore();
  await binder.setSceneWordCount(sceneId, countWords(text));
}

function restoreFailure(error: unknown): { title: string; message: string } {
  if (error instanceof PendingMobileSceneChangesError) {
    return {
      title: "Still saving",
      message: "Finish your edit, then try restoring this version again.",
    };
  }
  return {
    title: "Couldn't restore this version",
    message: "Please try again. Your saved versions are still available.",
  };
}
...
function Viewer(props: { snapshot: Snapshot; versionText: string; currentText: string; title: string; onRestore: () => Promise<boolean> }) {
...
export function VersionHistoryScreen({ navigation, route }: Props) {
  const theme = useTheme(); const { projectId, sceneId } = route.params; const [snapshots, setSnapshots] = useState<Snapshot[]>([]); const [title, setTitle] = useState("Scene"); const [currentWords, setCurrentWords] = useState(0); const [selectedId, setSelectedId] = useState<string | null>("snapshotId" in route.params ? route.params.snapshotId ?? null : null); const [versionText, setVersionText] = useState(""); const [currentText, setCurrentText] = useState(""); const [rename, setRename] = useState<Snapshot | null>(null);
  const load = useCallback(() => { void Promise.all([getSnapshotStore(), getBinderStore(), getSceneDocStore()]).then(async ([store, binder, docs]) => { const [list, project, current] = await Promise.all([store.listSnapshots(sceneId), binder.loadProject(projectId), docs.load(sceneId)]); const scene = project.scenes.find(({ id }) => id === sceneId); setSnapshots(list); setTitle(scene?.title ?? "Scene"); setCurrentWords(scene?.word_count ?? 0); setCurrentText(current ? plainText(current) : ""); const id = selectedId ?? list[0]?.id; if (id) { setSelectedId(id); const record = await store.getSnapshot(id); setVersionText(record ? plainText(record.stateBase64) : ""); } }); }, [projectId, sceneId, selectedId]);
  ...
  const capture = async () => { const [store, docs] = await Promise.all([getSnapshotStore(), getSceneDocStore()]); const stateBase64 = await docs.load(sceneId); if (!stateBase64) return; await store.takeSnapshot({ sceneId, label: null, stateBase64, wordCount: currentWords, kind: "manual" }); load(); };
  const restore = async (): Promise<boolean> => { if (!selected) return false; const [store, docs, owner] = await Promise.all([getSnapshotStore(), getSceneDocStore(), getEpochOwner()]); mobileEngine.pause(); try { const restored = await restoreSnapshotSafely({ getSnapshot: (id) => store.getSnapshot(id), readCurrentScene: async (id) => { const stateBase64 = await docs.load(id) ?? encodeDoc(new Y.Doc()); return { stateBase64, wordCount: countWords(plainText(stateBase64)) }; }, takeSafetySnapshot: async (input) => (await store.takeSnapshot({ ...input, label: null, kind: "auto" })).id, publishSnapshot: async () => undefined, replaceThroughEpoch: async (input) => { await owner.replaceThroughEpoch({ ...input, plaintext: plainText(input.stateBase64) }); await updateRestoredWordCount(input.sceneId, input.stateBase64); }, replaceActiveScene: (input, persist) => replaceActiveMobileSceneDurably(input.sceneId, input.stateBase64, persist) }, { projectId, sceneId, snapshotId: selected.id }); if (!restored) return false; } catch (error: unknown) { const failure = restoreFailure(error); Alert.alert(failure.title, failure.message); return false; } finally { mobileEngine.resume(); } try { await mobileEngine.syncNow(); } catch { /* Durable outbox retry owns relay recovery. */ } load(); return true; };
  return <Screen contentStyle={styles.screen}><Topbar breadcrumb={`${title} · your current draft is never touched until you restore`} leading={<IconButton icon="chevLeft" label="Back" onPress={navigation.goBack} />} title="Version history" />{snapshots.length === 0 ? <View style={styles.empty}><Icon color={theme.colors.ink4} name="rotate" size={46} strokeWidth={1.2} /><Text style={[TYPE.cardTitle, { color: theme.colors.ink }]}>No versions yet</Text><Text style={[TYPE.bodySmall, styles.emptyCopy, { color: theme.colors.ink2 }]}>Take a snapshot before a big change — you can compare and roll back any time.</Text><View style={styles.first}><PrimaryButton onPress={() => { void capture(); }}>Take first snapshot</PrimaryButton></View></View> : <ScrollView contentContainerStyle={styles.content}><View style={styles.listHead}><Text style={[TYPE.bodySmallStrong, { color: theme.colors.ink3 }]}>{snapshots.length} versions</Text><Pressable onPress={() => { void capture(); }} style={[styles.take, { backgroundColor: theme.colors.parchmentDeep }]}><Icon color={theme.colors.ink2} name="camera" size={14} /><Text style={[TYPE.meta, styles.takeText, { color: theme.colors.ink2 }]}>Take snapshot</Text></Pressable></View><ScrollView horizontal contentContainerStyle={styles.versions} showsHorizontalScrollIndicator={false}>{snapshots.map((snapshot) => <VersionCard currentWords={currentWords} key={snapshot.id} onLongPress={() => menu(snapshot)} onPress={() => { void select(snapshot); }} selected={selected?.id === snapshot.id} snapshot={snapshot} />)}</ScrollView>{selected && <Viewer currentText={currentText} onRestore={restore} snapshot={selected} title={title} versionText={versionText} />}<Text style={[TYPE.metaSmall, styles.longPress, { color: theme.colors.ink3 }]}>Long-press a version to rename or delete it.</Text></ScrollView>}<RenameSheet key={rename?.id ?? "closed"} onDismiss={() => setRename(null)} onSave={(value) => { if (!rename) return; void getSnapshotStore().then((store) => store.renameSnapshot(rename.id, value)).then(() => { setRename(null); load(); }); }} snapshot={rename} /></Screen>;
}
