...
import { Card, Icon, Meter, PrimaryButton, Screen, SecondaryButton, Topbar } from "../../components";
...
export function TrialScreen({ navigation }: Props) {
  ...
  return <Screen contentStyle={styles.screen}><Topbar title="Free trial" />
    <View style={styles.content}><Card style={styles.card}><View style={styles.head}><Icon color={theme.colors.accent} name="clock" size={20} />
      ...
      <View style={styles.actions}><PrimaryButton onPress={() => navigation.navigate("Activation", { reason: "missing" })}>Enter a key</PrimaryButton>
        <SecondaryButton onPress={() => { void Linking.openURL("https://writersnook.app/pricing"); }}>Buy a license</SecondaryButton></View></Card>
