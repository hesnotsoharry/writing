...
function DragHandle({ onDrop }: { onDrop: (translationY: number) => void }) {
  const theme = useTheme();
  const [dragging, setDragging] = useState(false);
  const gesture = useMemo(() => Gesture.Pan().activateAfterLongPress(350).runOnJS(true)
    .onStart(() => setDragging(true)).onEnd((event) => onDrop(event.translationY))
    .onFinalize(() => setDragging(false)), [onDrop]);
    ...
    <Pressable onPress={props.onActivate} style={[styles.row, { borderBottomColor: theme.colors.lineSoft }, props.active && { backgroundColor: theme.colors.accentTint }]}>
      ...
      <View style={styles.main}>
        <View style={styles.titleRow}>
          ...
          <DragHandle onDrop={props.onDrop} />
        </View>
...
const styles = StyleSheet.create({
  row: { minHeight: 96, flexDirection: "row", paddingHorizontal: 12, paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth },
