...
export function NewGoalScreen({ navigation, route }: Props) {
  ...
  return <Screen contentStyle={styles.screen}><Topbar leading={<Pressable onPress={navigation.goBack} style={styles.topAction}><Text style={[TYPE.bodySmallStrong, { color: theme.colors.ink3 }]}>Cancel</Text></Pressable>} title="New goal" trailing={<Pressable disabled={saving} onPress={() => { void save(); }} style={styles.topAction}><Text style={[TYPE.bodySmallStrong, { color: saving ? theme.colors.ink4 : theme.colors.accent }]}>Save</Text></Pressable>} /><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Text style={[TYPE.sectionLabel, { color: theme.colors.ink3 }]}>What are you tracking?</Text><GoalTypePicker onSelect={selectType} selected={type} /><Text style={[TYPE.sectionLabel, styles.targetLabel, { color: theme.colors.ink3 }]}>Target</Text><TargetCard countDaysOff={countDaysOff} draft={draft} onDaysOff={setCountDaysOff} setDraft={setDraft} type={type} /><Text style={[TYPE.meta, styles.footer, { color: theme.colors.ink3 }]}>You can run several goals at once — a daily count and a deadline pace work well together.</Text></ScrollView></Screen>;
}
