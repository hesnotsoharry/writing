...
export function NewGoalScreen({ navigation, route }: Props) {
  ...
  const save = async () => { if (saving) return; setSaving(true); try { const binder = await getBinderStore(); const project = await binder.loadProject(projectId); const words = project.scenes.reduce((sum, scene) => sum + scene.word_count, 0); const write = goalWrite(type, { ...draft, current: words, startWords: words }, words, countDaysOff); const store = await getGoalsStore(); await store.upsertGoal({ projectId, goalType: write.goalType, target: write.target, enabled: write.enabled, config: write.config }); navigation.goBack(); } finally { setSaving(false); } };
  return <Screen contentStyle={styles.screen}><Topbar leading={<Pressable onPress={navigation.goBack} style={styles.topAction}><Text style={[TYPE.bodySmallStrong, { color: theme.colors.ink3 }]}>Cancel</Text></Pressable>} title="New goal" trailing={<Pressable disabled={saving} onPress={() => { void save(); }} style={styles.topAction}><Text style={[TYPE.bodySmallStrong, { color: saving ? theme.colors.ink4 : theme.colors.accent }]}>Save</Text></Pressable>} /><KeyboardAwareScrollView bottomOffset={KEYBOARD_BOTTOM_OFFSET} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Text style={[TYPE.sectionLabel, { color: theme.colors.ink3 }]}>What are you tracking?</Text><GoalTypePicker onSelect={selectType} selected={type} /><Text style={[TYPE.sectionLabel, styles.targetLabel, { color: theme.colors.ink3 }]}>Target</Text><TargetCard countDaysOff={countDaysOff} draft={draft} onDaysOff={setCountDaysOff} setDraft={setDraft} type={type} /><Text style={[TYPE.meta, styles.footer, { color: theme.colors.ink3 }]}>You can run several goals at once — a daily count and a deadline pace work well together.</Text></KeyboardAwareScrollView></Screen>;
}
