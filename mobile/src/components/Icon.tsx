...
export const ICON_PATHS = {
  ...
  focus: '<path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/>',
  ...
  cloud: '<path d="M17.5 19a4.5 4.5 0 0 0 .5-8.97A6 6 0 0 0 6.34 9.5 4 4 0 0 0 6.5 19z"/><polyline points="9 13 11 15 15 11"/>',
  ...
  wifiOff: '<line x1="2" y1="2" x2="22" y2="22"/><path d="M8.5 16.5a5 5 0 0 1 7 0"/><path d="M2 8.82a15 15 0 0 1 4.17-2.65"/><path d="M10.66 5c4.01-.36 8.14.9 11.34 3.76"/><line x1="12" y1="20" x2="12.01" y2="20"/>',
  cloudPlain: '<path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"/>',
} as const;

export type IconName = keyof typeof ICON_PATHS;

export function getIconElements(name: IconName): string[] {
  return ICON_PATHS[name].match(/<(?:path|line|polyline|polygon|rect|circle)\b[^>]*\/?\s*>/g) ?? [];
}
...
function iconXml(path: string, color: string, strokeWidth: number): string {
  const content = path.replaceAll("currentColor", color);
  return `<svg viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round">${content}</svg>`;
}

export function Icon({ name, size = 24, color, strokeWidth = 1.7 }: IconProps) {
  const theme = useTheme();
  const resolvedColor = color ?? theme.colors.ink;
  return <SvgXml xml={iconXml(ICON_PATHS[name], resolvedColor, strokeWidth)} width={size} height={size} />;
}
