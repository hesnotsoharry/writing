// Mobile mirror of src/db/sqliteMetaApplyTarget.ts, over the mobile DbClient.
// Implements MetaApplyTarget (src/sync/meta/applyExec.ts) so the shared
// SyncEngine can project an incoming project-meta Yjs doc into this device's
// SQLite binder tables (projects/folders/scenes/labels/scene_labels). SQL is
// identical to desktop's — mirrored, not shared: the desktop class imports
// Tauri-bearing schema.ts (S4 blueprint portable-boundary rule).
import type { MetaApplyTarget } from "../shared/applyExec";
import type {
  DeleteOp, LabelOp, SortOrderRewrite, SqlProjectionSnapshot,
} from "../shared/applyPlan";
import type { DbClient } from "../shared/dbClient";
import type { MetaProject } from "../shared/metaDoc";
...
async function loadProjection(db: DbClient, projectId: string): Promise<SqlProjectionSnapshot> {
  ...
  return {
    ...
    labels: labels.map((row) => ({
      ...row, color: row.color as SqlProjectionSnapshot["labels"][number]["color"],
    })),
    sceneLabels,
  };
}
...
export class MobileMetaApplyTarget implements MetaApplyTarget {
  ...
  async load(projectId: string): Promise<SqlProjectionSnapshot> {
    return loadProjection(await getMobileDb(), projectId);
  }
  ...
  async delete(op: DeleteOp): Promise<void> { await applyDelete(await getMobileDb(), op); }
