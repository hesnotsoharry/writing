...
export function encodeTargetId(targetId: string | null): string {
  return targetId ?? "_";
}

export function baselineKey(key: ScopedGoalKey, date: string): string {
  return `writing.goal.baseline.${key.projectId}.${key.scope}.${encodeTargetId(key.targetId)}.${date}`;
}

export function metKey(key: ScopedGoalKey, date: string): string {
...
export interface GoalStorageReader {
  getItem(key: string): string | null;
}

export function readMonthlyMetDaysFrom(
  storage: GoalStorageReader,
  key: ScopedGoalKey,
  year: number,
  month: number,
): Set<number> {
  ...
  for (let day = 1; day <= daysIn; day++) {
    ...
    if (storage.getItem(metKey(key, date)) === "1") met.add(day);
  }
  return met;
}

export function readDailyWordsFrom(
  storage: GoalStorageReader,
  key: ScopedGoalKey,
  currentTotal: number,
  date: string,
): number {
  const stored = storage.getItem(baselineKey(key, date));
  if (stored === null) return 0;
  const baseline = parseInt(stored, 10);
  return Math.max(0, currentTotal - (Number.isFinite(baseline) ? baseline : currentTotal));
}

export function goalStreakFrom(
  storage: GoalStorageReader,
  key: ScopedGoalKey,
  now: number,
): number {
  ...
  for (let offset = 0; offset < 3650; offset++) {
    const date = new Date(now - offset * msPerDay).toLocaleDateString("sv");
    if (storage.getItem(metKey(key, date)) === "1") streak++;
    else break;
  }
  return streak;
}
