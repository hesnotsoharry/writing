...
import { LabelManagerOverlay, useLabelState } from "./App.content.labels";
...
import type { LabelStore } from "./db/labelStore";
...
function useCaretInserter() {
  const ref = useRef<((text: string) => void) | null>(null);
  const onRegister = useCallback((fn: (text: string) => void) => { ref.current = fn; }, []);
  const onInsert = useCallback((name: string) => { ref.current?.(name); }, []);
  return { onRegisterInsert: onRegister, onInsertAtCaret: onInsert };
}
...
function useBrainstormState(activeProjectId: string | null, view: AppView, onViewChange: (v: AppView) => void) {
  ...
  if (prevProjectId !== activeProjectId) {
    ...
    if (view === "brainstorm") onViewChange("editor");
  }
  return { selectedBoardId, setSelectedBoardId };
}

function useContentMetrics(props: AppContentProps, liveWordCount: number) {
  const { tree, selectedSceneId, projects, activeProjectId, archivedVersion } = props;
  ...
  return { manuscriptTotal, goalProgress, quickCount, archivedCount, docName, activeScene, chapterId, chapterTotal };
}
...
function useAppContentSlots(props: AppContentProps) {
  ...
  const metrics = useContentMetrics(props, liveWordCount);
  ...
  const { selectedBoardId, setSelectedBoardId } = useBrainstormState(activeProjectId, view, onViewChange);
  const showSidePanels = !focusMode && view !== "cork" && view !== "outline" && view !== "bible" && view !== "entry";
  const { onRegisterInsert, onInsertAtCaret } = useCaretInserter();
  const selectSceneWithViewReset = useCallback((id: string) => { if (view === "brainstorm") onViewChange("editor"); onSelectScene(id); }, [view, onViewChange, onSelectScene]);
  const { binderSlot, inspectorSlot } = buildSideSlots({
    ...props, ...metrics, overlays, liveWordCount, onAddGoal, onExport, onGoalMenu: openGoalMenu,
    showSidePanels, onSelectScene: selectSceneWithViewReset, onInsertAtCaret, selectedBoardId, aiEnabled, onSetSceneExcludedFromAi: callbacks.onSetSceneExcludedFromAi,
    onOpenBrainstorm: (boardId: string) => { setSelectedBoardId(boardId); onViewChange("brainstorm"); } });
  const { onDeleteEntity } = makeEntityHandlers(storyBibleStore, props.onEntitiesChanged);
  ...
  const onFindMentions = (n: string) => { setFindReplaceSeed?.(n); setShowFindReplace(true); };
  const viewStageContent = makeViewStage({
    ...props, ...metrics, ls, onDeleteEntity, onAddGoal, onArchiveScene: callbacks.onArchiveScene, onExport, editorFocus, onFindMentions, onRegisterInsert, brainstormBoardId: selectedBoardId, onRenameScene: callbacks.onRenameScene, onSetSceneStatus: callbacks.onSetSceneStatus, onSetSceneExcludedFromAi: callbacks.onSetSceneExcludedFromAi, onSelectScene: selectSceneWithViewReset, onTakeSnapshot: callbacks.onTakeSnapshot, onOpenHistory: callbacks.onOpenHistory,
  });
  return {
    focusMode, setFocusMode, goalsOn, hasQuickItems, setShowGoals, setShowQuickCapture, setShowSettings,
    setShowExport, openExport, liveWordCount, manuscriptTotal: metrics.manuscriptTotal, goalProgress: metrics.goalProgress, docName: metrics.docName,
