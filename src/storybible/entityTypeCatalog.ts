...
export interface EntityTypeDef {
   ...
   */
  color: string;
  /** Native-renderable accent token; null for the generic non-label fallback. */
  paletteToken: LabelColor | null;
}
...
export const ENTITY_TYPE_DEFS: Readonly<Record<string, EntityTypeDef>> = {
  character: { label: 'Character', icon: 'user',    color: 'var(--label-clay)',  paletteToken: 'clay'  },
  location:  { label: 'Location',  icon: 'mapPin',  color: 'var(--label-moss)',  paletteToken: 'moss'  },
  item:      { label: 'Item',      icon: 'box',     color: 'var(--label-gold)',  paletteToken: 'gold'  },
  faction:   { label: 'Faction',   icon: 'flag',    color: 'var(--label-plum)',  paletteToken: 'plum'  },
  lore:      { label: 'Lore',      icon: 'globe',   color: 'var(--label-sea)',   paletteToken: 'sea'   },
  theme:     { label: 'Theme',     icon: 'sparkle', color: 'var(--label-slate)', paletteToken: 'slate' },
};
...
function toPaletteToken(color: string): LabelColor | null {
  return LABEL_COLORS.includes(color) ? color as LabelColor : null;
}
...
export function resolveEntityTypeDef(
  ...
  if (custom) {
    ...
    return { label: custom.name, icon, color: custom.color, paletteToken: toPaletteToken(custom.color) };
  }

  return { label: typeName, icon: 'circleOpen', color: 'var(--ink-3)', paletteToken: null };
}
