...
export interface StatusMeta {
  id: SceneStatus;
  label: string;
  /** CSS color string for the status glyph / dot. */
  dot: string;
  /** Icon name — matches IconName in components/Icon.tsx. */
  icon: string;
  /** True only for "final". */
  isFinal: boolean;
}

// Dot colors and icon names match design-reference/data.jsx STATUS_META exactly.
export const STATUS_META: Record<SceneStatus, StatusMeta> = {
  blank:   { id: "blank",   label: "To write",  dot: "var(--ink-4)",  icon: "circleOpen", isFinal: false },
  outline: { id: "outline", label: "Outlined",  dot: "var(--note)",   icon: "list",       isFinal: false },
  draft:   { id: "draft",   label: "Drafting",  dot: "var(--accent)", icon: "pencil",     isFinal: false },
  revise:  { id: "revise",  label: "Revising",  dot: "#6a86a8",       icon: "rotate",     isFinal: false },
  final:   { id: "final",   label: "Final",     dot: "var(--good)",   icon: "check",      isFinal: true  },
};
