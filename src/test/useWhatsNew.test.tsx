 ...
 * bump with real CHANGELOG.md notes shows and defers the store until
...
describe("useWhatsNew", () => {
  it("fresh install: stores the current version silently, never opens", async () => {
    mockGetVersion.mockResolvedValue("0.12.9");
    const { result } = renderHook(() => useWhatsNew(false));

    await waitFor(() => {
      expect(localStorage.getItem(LAST_SEEN_KEY)).toBe("0.12.9");
    });
    expect(result.current.open).toBe(false);
  });

  it("version change with real CHANGELOG.md notes opens, and dismiss stores the version", async () => {
    localStorage.setItem(LAST_SEEN_KEY, "0.12.8");
    mockGetVersion.mockResolvedValue("0.12.9");
    const { result } = renderHook(() => useWhatsNew(false));

    await waitFor(() => expect(result.current.open).toBe(true));
    expect(result.current.version).toBe("0.12.9");
    expect(result.current.notes).toContain("What's new");
    // Not stored yet — only on dismiss.
    expect(localStorage.getItem(LAST_SEEN_KEY)).toBe("0.12.8");
  ...
  it("same version as last seen: no store, no open", async () => {
    localStorage.setItem(LAST_SEEN_KEY, "0.12.9");
    mockGetVersion.mockResolvedValue("0.12.9");
    const { result } = renderHook(() => useWhatsNew(false));
