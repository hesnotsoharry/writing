...
async function sqliteWriteReciprocal(
  db: DbClient,
  write: ReciprocalWrite,
): Promise<string | null> {
  const { projectId, args, forwardId, createdAt } = write;
  ...
  if (invExisting.length === 0) {
    ...
    await db.execute(
      ...
      [invId, projectId, toEntity, fromEntity, reciprocalLabel, forwardId, createdAt]
    );
    await db.execute(`UPDATE entity_relations SET reciprocal_id = $1 WHERE id = $2`, [invId, forwardId]);
    return invId;
  }
  const reciprocalId = invExisting[0].id;
  await db.execute(`UPDATE entity_relations SET reciprocal_id = $1 WHERE id = $2`, [reciprocalId, forwardId]);
  return reciprocalId;
}

// ── sqliteAddRelation ─────────────────────────────────────────────────────────

export async function sqliteAddRelation(db: DbClient, projectId: string, args: AddRelationArgs): Promise<Relation> {
  ...
  const reciprocalId = await sqliteWriteReciprocal(db, {
    projectId,
    args,
    forwardId: fwdId,
    createdAt: now,
...
export async function sqliteListRelations(db: DbClient, projectId: string, entityId?: string): Promise<Relation[]> {
  let rows: RelRow[];
...
export async function sqliteDeleteRelation(db: DbClient, id: string): Promise<void> {
...
export async function sqliteUpdateRelationLabel(db: DbClient, id: string, label: string): Promise<void> {
