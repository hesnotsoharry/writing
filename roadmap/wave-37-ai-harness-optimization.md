...
- [ ] `VERB_CONFIG` exists server-side and `chat.ts` resolves `model` / `temperature` / `max_tokens` from it per verb; the proxy no longer uses a client-sent `max_tokens` as the cap (grep `chat.ts`: `body.max_tokens`/equivalent is not the request cap).
- [ ] A missing or invalid `verb` resolves to a documented safe default (test asserts the fallback config, not a 500).
- [ ] `credits.ts` exposes `RATES` keyed by model with `input` / `output` / `cacheWrite` / `cacheRead`; `estimateCredits` and `actualCredits` both accept the model.
- [ ] A unit test exercises `actualCredits` for every (model × {no-cache, cache-write, cache-read}) combination and asserts correct unit math (the billing-correctness gate).
- [ ] Per-verb temperature is resolved server-side — brainstorm 1.0 / beta-read 0.7 / proofread 0.1 / critique 1.0 (test asserts the resolved config); when a verb sets `thinking`, `temperature` is omitted (mutual-exclusion test).
- [ ] Per-verb `max_tokens` resolved server-side — brainstorm + beta-read 2048, proofread ≥ 4096 (test asserts resolved config).
- [ ] `prompts/shared.ts` contains a shared `<principles>` anti-sycophancy block referenced by all four verb builders; critique retains its three-header structure (grep the three headers).
- [ ] `buildGrounding` emits a truncation notice when the scene excerpt is truncated — test asserts the notice appears for >2000-char input and is absent for short input.
- [ ] `assembleBrainstormContext` is removed or routed through `filterAiEntities`; grep shows no exported unfiltered entity-assembly path remains.
- [ ] `cache_control` breakpoints are present on the system + manuscript-context blocks in the proxy request, and caching is gated on the Haiku 4096-token minimum (test or code-path assertion).
- [ ] No verb's model changed from Haiku this wave (grep: every `VERB_CONFIG` model = `claude-haiku-4-5-20251001`).
- [ ] `npm run test` (repo root) and `npm run test` inside `marketing/` both pass; `tsc --noEmit` and `npm run lint` clean in both trees.
...
<!-- DEFAULT: empty. Tier-3 triple-gate only (VALUE w/ present-harm pointer + STRUCTURAL + CLEARABILITY). Below the bar → fix in-wave. -->
