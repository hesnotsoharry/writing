...
| 8 Animations + chrome | 2026-06-04 | 2026-06-04 | 8db0e77 | gates green (587 tests, +16). Root cause confirmed: AppShell never stamped .anim on .win — every .anim-gated CSS rule in app.css was permanently dead. Fix: useMotion hook reads motion tweak + prefers-reduced-motion → passed as anim prop to AppShell → .win.anim applied. Also wired viewKey on .view-stage so React remounts the wrapper on view change, restarting view-in CSS entrance animation. WindowControls: replaced inline styles with canon .wbtn/.wbtn.close CSS classes so hover CSS targets correctly. app.css: close hover changed from #d6573e to var(--accent) (clay token). (a) view-in fade-up active for Write/Cork/Bible. (b) overlay pop/float-in confirmed unconditional in .sheet CSS (already working). (c) bar-in selected-indicator .anim-gated, fires once .anim set. (d) close button hover clay. Runtime observation: no Tauri runtime available — verified via .anim gating logic, class application, and 16 tests (+3 AppShell anim class, +2 WindowControls wbtn class, +existing). |
