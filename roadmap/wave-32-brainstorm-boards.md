...
| 4 | 2026-06-10 (run-phase `wf_238c1ee5-522`, single tier; prior SHA 78674e4; oracle committed pre-dispatch at 78674e4) | 2026-06-11 | 4806b57 | DEFERRED — entity cards require a running app with at least one entity in the Story Bible; rename propagation path (window.focus re-read) is non-automatable from a context-resumed session without a live Tauri dev build. Observation point (Cole adds a character card, renames that character, returns to board and sees new name) requires human CDP or manual verify at next dev session. oracle 37/37 pass; lint + tsc green. |
