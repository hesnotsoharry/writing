...
| 4 | 2026-06-10 (run-phase `wf_238c1ee5-522`, single tier; prior SHA 78674e4; oracle committed pre-dispatch at 78674e4) | 2026-06-11 | 4806b57 + fix commit | YES (orchestrator CDP smoke, superseding implementer's DEFERRED note) — picker lists entities, closes on pick; entity card renders live name "test char 1 · Character" with real type color (post custom-types fix); injected dangling entityRef renders "Missing entity" placeholder; both persist across full reload with name re-resolved from live store (proves reference-not-copy). Reviewer FLAG (hardcoded empty customTypes at EntityCardNode:79) FIXED — mirrors BibleListView's listEntities+listCustomTypes read. Reviewer FLAG_UNCERTAIN (picker toggle race) CONFIRMED live via trusted CDP click and FIXED (excludeRef on outside-mousedown handler); toggle open→close re-verified live. Rename-propagation via window.focus re-read = human-verify (same read path the reload re-resolution proved). Note: implementer self-committed 4806b57 (protocol drift — commits are orchestrator-owned; harmless, not redone). |
| 5 | 2026-06-11 (run-phase `wf_878104e5-242`, PANEL tier; prior SHA 5c789e0; orchestrator-authored hot/cold oracle committed pre-dispatch at 5c789e0 — 10 tests) | 2026-06-11 | 719f655 + fix commit | YES — panel: FLAG/FLAG_UNCERTAIN/PASS, constructive FIT, oracle 10/10. Adjudication: (1) cold-path null plaintext → FIXED (extractPlainText(tmpDoc), mirrors promoteNoteToScene); (2) init-only onSendToSceneRef → JUSTIFIED (useCallback([]) = permanently stable identity; project's react-hooks/refs lint FORBIDS the render-body write the reviewer wanted — comment added documenting the constraint); (3) oracle-file import-order lint → fixed by orchestrator (owns the oracle). CDP smoke HOT PATH END-TO-END: opened scene test1 (live doc at App level), sent card from board, editor showed appended text WITHOUT reload, typed KEYSTROKE-AFTER-SEND, full reload → BOTH append and keystroke survived (Decision 3 data-loss scenario closed). Send button renders on text cards only (entity cards excluded, verified in a11y tree). Implementer self-committed 719f655 (same protocol drift as P4). |
| 6 | 2026-06-11 (run-phase `wf_4e2ff255-0d3`, single tier — consumes existing creation APIs only, no new persistence contract; oracle committed pre-dispatch at 5cb44af — 17 tests) | 2026-06-11 | (this commit) | YES — CDP smoke: promote menu (→ New scene / → New entity…) on text cards; promote-to-scene created the scene (title from first line, ProseMirror-compatible content), navigated to editor with text carried; graduated card renders dimmed (0.55, `card-node--graduated`), send/promote affordances removed, "→ <scene title>" link navigates to the destination; graduation persists across reload; card content kept (provenance). Reviewer FLAG adjudicated: title truncation aligned to spec (40 chars; "New Scene" fallback kept — app-idiomatic, justified); entity promote now carries card body into entity notes (was silently discarded); promote handlers now .catch + log (were unhandled rejections); import-spacing fixed (cosmetic — lint had passed). **Found via smoke + fixed at source: `bindPersistence` unbind CANCELLED pending debounced saves — any write within 500ms of unmount was silently dropped (graduation was lost on promote-navigate; same latent class affected final keystrokes before scene/view switches). Unbind now FLUSHES the pending save; bindPersistence test updated to pin the new contract.** |
