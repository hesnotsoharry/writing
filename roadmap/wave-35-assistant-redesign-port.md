...
| D | 2026-06-12 (run-phase `wf_22317822-a0e`, panel tier) | 2026-06-12 | 894c2fd | PARTIAL: persistence contract test-proven (oracle "survives restart across store instances" — write w/ store A, read w/ store B on same DB — passes 8/8); full suite 1309 green. Live UI persistence smoke (list-on-mount, delete-in-UI, reload-survives) DEFERRED — dev app CDP port dropped after Phase C smoke (later-phase CANNOT-LAUNCH → graceful-degrade per wave-process; fold into next smoke when app restarted). Panel FLAGs (done-event credits_cost, persistSend stuck-send, new-conv listMessages race) all fixed + re-verified. |
| E | 2026-06-13 (run-phase `wf_d178f575-dd8`, single tier) | 2026-06-13 | (this commit) | Oracle 9/9 (D4 privacy: shielded + off-entities dropped inside assembleContext; About/selection/boundary inclusion); full suite 1318 green; adversarial PASS all 4 angles. behavioralCoverageGap flagged on new SQLite reads (getManuscriptAbout/getSceneText) → backfill test dispatched. Live receipt-chip smoke folded into the D+E consolidated CDP pass. |
| F | — | — | — | — |
