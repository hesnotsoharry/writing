...
| I | 2026-06-13 (run-phase `wf_752ef4f8-79b`, single tier) | 2026-06-13 | (this commit) | Selection affordances: parseProseSelection (oracle 6/6), useProseSelection (DOM selectionchange + .prose guard per locked decision — NOT editor.selectionUpdate), AiAskPill (portal, onMouseDown), AiSlot seed/panelKey key-remount + live sel chip, right-click AI items (gated aiEnabled && aiSelMenu) dispatching AI_ASK_FROM_EDITOR. Full suite 1376 green. Reviewer FLAG_UNCERTAIN (speculative zero-rect on hidden selection — not reproducible in normal TipTap; justified-proceed). HIT (orchestrator CDP smoke): a ≥3-word .prose selection rendered the AiAskPill ("✦ Ask the assistant · 147w", correct word count, gated on aiSelPill); invoking its onMouseDown switched the inspector to the Assistant tab. Composer-seed-with-chip is deploy-gated (offline fallback blocks compose until balance.ts deploys on ship-push) — same gate as the meter. |
