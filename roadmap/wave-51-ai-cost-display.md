...
| 2 | Server surfaces true per-request charge to the client | `sonnet-implementer` | pyramid · cross-boundary (proxy response contract) · `actualCredits` is already computed; add a `chargedUnits` field to the stream done payload (and the non-stream response). No new cost math — surface existing truth. test + tsc gates only. reviewTier: single | The proxy `done` event/response carries `chargedUnits` reflecting the real charge incl. cache. (network inspection + a marketing test asserting the field) |
| 3 | Client cost restructure: rolling-average assists, honest pre-send estimate, post-send truth | `sonnet-implementer` | trophy · cross-boundary (consumes P2 field) · Assists-remaining → rolling avg of `balance_before − balance_after` (localStorage window ~5–10, per-model cold-start fallback). Context-modal meter → model-aware `(ctxTokens×rate.in + 800×rate.out)/monthlyAllowance`, labelled "≈ up to X%" (worst-case/no-cache); thread selected model + live `monthlyAllowance` into `AiOverlays`. Post-send: show P2's `chargedUnits` as true cost. reviewTier: single | In the running app: after a reply the panel shows the real cost; assists-remaining tracks actual burn; the context modal shows "≈ up to X%" (model-aware, moves with context size). (CDP self-smoke) |
...
- [ ] The proxy response/stream `done` event includes `chargedUnits` equal to the server's `actualCredits` for that request.
- [ ] Assists-remaining is computed from a rolling average of observed balance deltas (verified by a unit test feeding synthetic deltas), with a per-model cold-start fallback when the window is empty.
...
**Consequences:** Client must consume a new server `chargedUnits` field (P2→P3 dependency); the static `TYPICAL_REQUEST` estimate and `words/4000` meter are retired; pre-send numbers are explicitly framed as estimates, never as charges.
