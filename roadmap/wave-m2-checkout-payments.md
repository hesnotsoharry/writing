...
**Browser smoke (2026-06-04, chrome-devtools) — validated the full checkout end-to-end against the live test store; caught + fixed two launch blockers:**
1. Stale `#discRow` + `#subNote` ("then $5.00/mo") rendered despite the `hidden` attr because `.co-trow{display:flex}` overrides it (the old mock toggled them via JS, removed in P1). Both dead rows deleted.
2. lemon.js never initialized: the URL `app.lemonsqueezy.com/js/lemon.js` now serves an **HTML meta-refresh redirect** (breaks as a `<script src>`); real lib is `assets.lemonsqueezy.com/lemon.js`. Also the current API requires `window.createLemonSqueezy()` before `window.LemonSqueezy` exists. Fixed both in checkout.html + checkout.js.

After the fixes: clicking Pay opens the real LS overlay (test mode) with the **$29** product and the **email prefilled** from our form. Real checkout-URL identifier is the variant's **UUID** (`6e07b36b-…`), not the numeric variant ID (1748920 = API/webhook id) — wired in `ls-config.js`. Coupon param passes through (no `FOUNDERS` coupon created in the dashboard yet).
