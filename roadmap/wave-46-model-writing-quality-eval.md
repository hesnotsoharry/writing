...
## Decision 14: Scorer Component-1 bracket scoring — flat brackets where the design gives no "interpolate" instruction (durable: candidate)
**Context:** P3 attack-diff review surfaced a genuine spec contradiction in `wave-46-scorer-design.md` §1: the sub-detector tables give discrete bracket scores ("MTLD 45–75 → 2.5"), but some brackets also say "interpolate linearly." The implementer interpolated TTR + MTLD (which have NO interpolate instruction), collapsing MTLD's 3 bands into a 2-band ramp that scored clearly-AI text (MTLD=75) at 0.57 instead of 2.5 — a BLOCK-level mis-ranking of the eval's primary instrument.
**Pick:** Where the design provides a discrete bracket table with **no** "interpolate" directive (TTR §1d-i, MTLD §1d-ii, transition-word §1c-ii) → **flat step functions** using the table's exact scores. Where it explicitly says interpolate (std-dev "within the suspicious band" §1c-i; negation 2-point §1c-iii) → interpolation stays. Cliché density §1b says "interpolate between brackets" but that contradicts its own flat table ("1–5 → 1.0") → **honor the concrete table (flat)**; interpolation shape deferred to the post-pilot calibration pass (§4).
**Rationale:** All Component-1 thresholds ship `provisional` and are precision-recall-tuned post-pilot (design §4/§5c) — exact interpolation curve is a calibration-time refinement, not a pilot requirement; the discrete bracket values are what the wordlist research actually pins. Flat-everywhere (except the specified-interpolation detectors) is table-faithful, consistent, and fully testable. Not architect-tier (provisional eval threshold, not a production config value) → orchestrator adjudication, no decision-review cell.
**Consequences:** MTLD/TTR/transition become flat 3-band lookups; cliché density flat per its table; std-dev + negation keep their specified interpolation. Pin tests lock the AI-band midpoints so a future threshold drift fails CI.
**Enforcement:** `src/test/scorer.test.ts` pin tests (`scoreMTLDValue(60)≈2.5`, `scoreMTLDValue(75)≈2.5`, `scoreTTRValue(0.20)≈3.0`, cliché 1 match/100w ≈ 1.0) — mechanical; + advisory design-doc note. durable: candidate (the flat-vs-interpolate rule outlives the pilot into the calibrated scorer).
