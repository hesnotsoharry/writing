...
| 3 | ✓ | ✓ | c4af305 | "Ask assistant" folded into FormatBubble (single bubble, removes AiAskPill overlap); AI action consent-gated (`aiEnabled && aiConsentGiven && aiSelPill`), formatting + AiExcludeToggle ungated; editor-ask inherits Phase-2 redaction via `extractAiSafeSelection` (review PASS on the security angle); 6/6 tests green; single-bubble visual confirm deferred to batched CDP smoke |
| 4 | ✓ | ✓ | 5ae6017 | migration #18 adds `exclude_from_ai` to characters+locations; `sqliteSetEntityExclusion` free-fn routes write by type; `rowToEntity` read-path surfaces the real flag (was hardcoded — the original bug); in-memory parity + interface + UI `toggleNever` rewire with refresh-key re-read. Oracle (entityExclusionPersist) exercises write→`sqliteListEntities`→`rowToEntity` round-trip for all 3 tables; review FLAG (oracle didn't hit `rowToEntity`) closed by extracting `sqliteListEntities` + adding a real-read-path case; full suite green (modulo 6 known-unrelated W46-eval failures); toggle-persists confirm deferred to batched CDP smoke |
| 5 | ✓ | ✓ | 35d561b | SERVER: `isContentPolicyBlock(status,body)` (400/403 + policy-keyword regex) in the `!res.ok` branch → emits `{type:"content-blocked"}` (full reserve still refunded, raw upstream body logged for parser-tightening) else generic error. Research confirmed input blocks are pre-stream HTTP 400, not mid-stream SSE. CLIENT: `content-blocked` added to `NormalizedEvent`; handler surfaces a calm BYOK/local nudge (event switch extracted to `AssistantPanel.streamEvents.ts` for testability — all prior branches verbatim-preserved, verified). Orchestrator-authored server acceptance test (policy-400→content-blocked+refund; non-policy-400→stays generic) GREEN unmodified; client `aiContentBlocked` test GREEN. Marketing 303/303 + tsc; client tsc+eslint+suite green (modulo 6 known-unrelated W46-eval). **Live confirmation deferred** — needs a real blocked request post-deploy; the server log captures the real body to tighten the regex. |
...
**Status: COMPLETE — all 5 phases shipped to master (commits 4fdf4c3, 2bc4f13, c4af305, 5ae6017, 35d561b).** Target version v0.12.0 (client release via `publish.ps1` is Cole's task; not bumped/tagged here — see "deferred" below).

**What shipped:**
- **P1 — `AiExclude` "Hide from AI" mark** (`src/editor/extensions/AiExcludeExtension.ts`): TipTap v3 mark, redacted styling, FormatBubble toggle; persists through the Yjs delta round-trip.
- **P2 — Mark-aware context redaction** (the load-bearing privacy step): `src/yjs/serialize.ts` replaces each marked run with `[passage hidden by author]` (AI-visible, not silent delete) at the `assembleContext` chokepoint, covering BOTH `sceneExcerpt` and `selectionText` for all 4 AI paths; plain `extractPlainText` (export/word-count) untouched. A panel-tier review caught a CRITICAL raw-`s.toString()` selection leak — fixed + regression-tested.
- **P3 — Bubble overlap + consent gate:** "Ask assistant" folded into FormatBubble (AiAskPill removed → one bubble); AI action gated on `aiEnabled && aiConsentGiven && aiSelPill`; plain formatting stays ungated.
- **P4 — Entity-exclusion persistence:** migration #18 adds `exclude_from_ai` to characters+locations (the table that silently reverted before); `sqliteSetEntityExclusion` routes by type; `rowToEntity` surfaces the real flag; `toggleNever` rewired with a refresh-key re-read. Oracle exercises write→`sqliteListEntities`→`rowToEntity`.
- **P5 — Managed-refusal warning:** server `isContentPolicyBlock` (400/403 + policy-keyword regex, raw body logged) emits `{type:"content-blocked"}`; client surfaces a calm BYOK/local nudge. Anthropic input blocks confirmed pre-stream HTTP 400, not mid-stream SSE.

**Gates:** tsc 0, eslint 0 across all 21 source files, full suite green modulo 6 known-unrelated W46-eval failures (scorer×3, eval-runner×3 — separate agent's in-flight work). Marketing 303/303 + tsc.

**Durable decisions promoted:** `roadmap/decisions/0002-ai-prose-redaction-chokepoint.md`, `0003-managed-refusal-server-detection.md`, `0004-entity-exclusion-persistence-schema.md`. **Vendor gotchas:** `marketing/.claude/vendor-gotchas/anthropic.md` (input-moderation 400 shape) + `.claude/vendor-gotchas/tiptap.md` (read mark from Yjs delta, not node.marks).

**Deferred / next-session:**
- **Runtime CDP smoke of the client phases was NOT performed** (no live AI spend permitted on the shared prod DB + overnight finish). Unit oracles cover the logic, but green vitest ≠ working editor (per `editor-behavior-needs-cdp-smoke-not-jsdom`). Batched CDP smoke owed: P1 mark visual + Yjs-reload persistence; P3 single-bubble-on-selection + consent-gated AI button; P4 toggle-persists-across-relaunch.
- **P5 live confirmation** needs a real post-deploy content-policy block (Cole-run); the server log captures the raw upstream body to tighten the detection regex from real traffic.
- **Push authorized by Cole** ("get the wave finished and pushed") — overrides the wave-plan's original "HOLD push" note. Push deploys the marketing site + Phase 5 server proxy live; the client half ships when Cole runs `publish.ps1` (bump v0.12.0 in the 4 files + tag — mind the queued v0.11.0 release).
