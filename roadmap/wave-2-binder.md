...
| 3 — Project switcher | ✅ sonnet-implementer | ✅ lint 0 / 17 tests / tsc clean. New `ProjectSwitcher.tsx` (select + "+ New project" sentinel); `App.tsx` projects/activeProjectId state + setActiveProject sync helper + useProjectActions (switch/create). Adversarial review (single) FLAG → 3 addressed: atomic ref+state set in init (no CRUD-null window/StrictMode regression), error handling on switch/create, loadProject token guard (latest switch wins). Tests: store contract already covers list/create; switch wiring smoke-verified (no component-test infra — noted as possible follow-up). | ⏳ pending manual smoke (create 2nd project, switch, CRUD isolation, relaunch — user-run) |
| 4 — Drag-reorder | — | — | — |
