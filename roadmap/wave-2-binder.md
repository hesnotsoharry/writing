...
| 3 — Project switcher | ✅ sonnet-implementer | ✅ lint 0 / 17 tests / tsc clean. New `ProjectSwitcher.tsx` (select + "+ New project" sentinel); `App.tsx` projects/activeProjectId state + setActiveProject sync helper + useProjectActions (switch/create). Adversarial review (single) FLAG → 3 addressed: atomic ref+state set in init, error handling on switch/create, loadProject token guard. Lane B follow-on: duplicate seed under StrictMode (two concurrent seedIfEmpty both saw empty table) — fixed by memoizing seedIfEmpty (module promise, like getDb). | ✅ CONFIRMED (smoke): switcher lists projects, create + switch work, per-project CRUD isolation, persists across relaunch; single clean seed (dup fixed). |
| 4 — Drag-reorder | ✅ sonnet-implementer | ✅ lint 0 / 31 tests / tsc clean. dnd-kit (@dnd-kit/react+helpers+dom 0.4, React 19). Store: moveScene/moveFolder; pure computeReorder (renormalize-on-move) + 10 unit tests. UI: DragDropProvider, draggable scenes+chapters, DroppableSceneList (empty containers accept drops). Adversarial review (single) FLAG → 3 addressed: move()-helper-derived index (reorder-down correct, not raw target.index), empty Short-pieces/chapters droppable, @dnd-kit/dom declared explicitly. 5px threshold separates click from drag. | ⏳ pending manual smoke (drag within/between chapters + in/out of Short pieces, reorder chapters, relaunch — user-run) |
