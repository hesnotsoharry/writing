...
| D model-picker UX | sonnet · single | 2026-06-14 | (this commit) | CDP self-smoke DEFERRED (Cole / off-master — no deployed proxy). Structural: client `ManagedModel` union matches server `MANAGED_MODELS` exactly (6 ids); model chip + `ModelPop` (standard grouped by provider, premium pair behind off-by-default "~3× cost" disclosure, auto-expand when premium selected); pref persists via `setStoredTweak('aiModel')`, threads to `streamChat` options; 4 wiring tests + 14 pre-existing green; tsc/lint clean. Attack-diff FLAG (verb-chip didn't close model-pop) self-fixed. |

## Follow-up candidates

- Proofread + premium-model picker mismatch (cosmetic): the model chip shows the user's selected model even when verb=proofread, but the server always bills proofread at Haiku (`resolveModelConfig` proofread override). A user who picks Opus/GPT-5.5 then runs proofread sees the premium label but is charged Haiku-rate — billing is CORRECT, the UI is just potentially confusing. Consider greying the chip or a tooltip when verb=proofread. | present-harm: K2 — UX-confusion only, no billing impact (`src/features/ai/AssistantPanel.parts.tsx` model chip renders `p.model` unconditionally; server override at `marketing/functions/api/ai/chat.ts` resolveModelConfig, 2026-06-14).
- Live-proxy end-to-end smoke for OpenAI routing is unrun (off-master branch, no deployed Worker + no `OPENAI_API_KEY` in env). Before W44 ships: deploy the marketing proxy with `OPENAI_API_KEY` set, then CDP-smoke a GPT-5.4 assist confirming the reply streams + the live meter decrements at the GPT-5.4 rate (the Phase D Observation point). | present-harm: K3 — the OpenAI path has never executed against the real API; param-matrix MEDIUM-confidence items (max_completion_tokens 400-behavior, reasoning_effort:'none'+temperature) are unverified live (wave file watch-list, 2026-06-14).
