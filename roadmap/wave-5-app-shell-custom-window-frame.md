...
- Transparent / floating rounded-shadow window aesthetic: deferred from wave-5 to avoid the Windows WebView2 transparency render risk; requires `transparent:true` + shadow handling + Windows verification, separate from the functional shell. **Re-enabling step:** un-comment the `.win` `@media (min-width:1180px)` floating block in `src/styles/app.css` (disabled in wave-5 — it exposed the opaque `#cfc6b4` body as a cream frame without transparency) AT THE SAME TIME as setting `transparent:true`. | present-harm: K3 — the design's "Quiet Study" floating look (inset/rounded/shadow over the desktop) is not present in the square-frameless ship; restored with transparency.
- Screen inline-style shedding (per-screen port waves — Binder first, HANDOFF step 2): the reparented screens still wear their own pre-design inline styles, which (a) duplicate the shell panel borders (Binder `<nav>` has its own `border-right` on top of `.panel-binder`'s → doubled border) and (b) override the theme tokens (Binder `background:#fafafa` → white pane instead of `--paper`). Each screen sheds these and adopts tokens in its own port wave. | present-harm: K3 — `src/binder/Binder.tsx` inline `border-right` + `background-color` produce a double border and a white left pane against the themed shell (observed in wave-5 smoke 2026-06-03); same pattern latent in Editor/Inspector/StoryBible.
