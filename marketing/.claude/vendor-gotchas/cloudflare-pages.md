...
## Pages' bundled npm 10.9.2 crashes on a lockfile-less `npm install` — keep the lockfile

Source: release-day session 2026-09-15 (deployments `6a552440`, `78cfac65`, `6148aa45`
all failed on commit `93578a1`; fixed by the commit that added `marketing/package-lock.json`)

**Symptom:** the build log ends with
`npm error Cannot read properties of null (reading 'edgesOut')` during
`Installing project dependencies: npm install --progress=false`. Nothing in the repo
caused it — the previous good commit (`ce0e6cb`, 3 weeks earlier) fails the same way
when re-installed today, so it is registry drift (a new transitive peer in vitest's
`@vitejs/devtools-*` set) hitting a known npm 10 arborist bug in `#loadPeerSet`.
Without a lockfile every build resolves fresh and is exposed to this.

**What did NOT work:** the Pages build env var `NPM_VERSION=11` (set via the API on
project `writing`, production env — still present, harmless). The v2 build image
logged `Detected the following tools from environment: npm@10.9.2` and crashed
identically. Retrying the failed deployment also re-runs with the old config.

**What worked:** a `package-lock.json` generated by npm 11.6.2 in a fresh clone. With
the lock present npm 10.9.2 installs the same tree (vitest 4.1.11, wrangler 3.114.17)
without rebuilding the peer set, and the 356-test marketing suite passes on that tree.
`.npmrc` (`package-lock=false`, from `d36cf93` — Pages once choked on a dual esbuild
tree when clean-installing an older lock) was removed so the lock is honoured.

**Rule going forward:** commit lockfile changes with dependency changes. If a Pages
install ever fails on the lock, regenerate it with npm 11 in a fresh clone and verify
with `npx npm@10.9.2 install` before pushing — do not delete it again.
