// ============================================================================
// ORCHESTRATOR-OWNED ACCEPTANCE TEST — macOS waitlist endpoint.
// The implementer MUST make this pass and MUST NOT modify it.
//   POST /api/macos-waitlist { email }:
//     - valid    -> upsert into `macos_waitlist` (onConflict email so a
//                   re-submit is a benign success), returns 200 {ok:true}.
...
import { onRequestPost } from "./macos-waitlist";
...
function ctx(body: unknown) {
  return {
    request: new Request("https://writersnook.app/api/macos-waitlist", {
      method: "POST",
...
describe("POST /api/macos-waitlist", () => {
  beforeEach(() => {
    upsertCalls = [];
  });

  it("adds a valid email to the waitlist and returns ok", async () => {
    const res = await onRequestPost(ctx({ email: "nina@writer.com" }));
    expect(res.status).toBe(200);
    expect(upsertCalls).toHaveLength(1);
    expect(upsertCalls[0].table).toBe("macos_waitlist");
    expect(upsertCalls[0].row).toMatchObject({ email: "nina@writer.com" });
  });

  it("treats a duplicate submission as a benign success (upsert keyed on the email conflict)", async () => {
