...
## 2026-06-15 — W49 multi-provider BYOK: second per-provider keyring entry `byok-openai` under same service
Source: wave-49, commit 9c152ee
**Gotcha:** Wave 49 added OpenAI as a second BYOK provider alongside Anthropic. Both keys are stored under the same Windows Credential Manager service (`com.coles.writing`) but with distinct usernames: `byok-anthropic` and `byok-openai` (the original pattern holds). A common mistake is trying to store both keys under the same username, which overwrites the first. Additionally, the frontend tracks both keys via separate `byokKeys.anthropic` and `byokKeys.openai` booleans; only one provider can be active at a time based on the selected model.
**Workaround:** The Rust commands follow the W40 pattern: `Entry::new("com.coles.writing", "byok-anthropic")` for Anthropic and `Entry::new("com.coles.writing", "byok-openai")` for OpenAI. Never reuse usernames across providers. Test via Credential Manager (`Win+R` → `Control Panel → User Accounts → Credential Manager → Windows Credentials`) to verify both credentials are present under the service name and have distinct target-names (`byok-anthropic.com.coles.writing` and `byok-openai.com.coles.writing`). The JavaScript front-end state (`useByokKeys()` hook) tracks both separately; the provider discriminant (`useByokMode()`) returns `'anthropic' | 'openai' | null` based on which key(s) are set.
**Why:** The multi-provider design reuses the same keyring service to keep credentials organized under the app's identity, but distinguishes providers by the username field to allow side-by-side storage without collision.
